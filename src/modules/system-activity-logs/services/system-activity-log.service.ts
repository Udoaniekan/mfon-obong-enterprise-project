...
import { SystemActivityLog, SystemActivityLogDocument } from '../schemas/system-activity-log.schema';
...
export class SystemActivityLogService {
  ...
  async createLog(createSystemActivityLogDto: CreateSystemActivityLogDto): Promise<SystemActivityLog> {
    try {
      this.logger.log(`Creating log entry: ${createSystemActivityLogDto.action}`);
      ...
      this.logger.log(`Log entry created successfully with ID: ${savedLog._id}`);
      
      return savedLog;
    } catch (error) {
      this.logger.error(`Error creating log entry: ${error.message}`, error.stack);
      throw error;
    }
  }
